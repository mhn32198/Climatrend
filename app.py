...
def create_plotly_figure(df, col_name, thresholds, resolution, pub_mode=False):
    ...
    if slope_stat is not None:
        ...
        stats_text = f"<br><br><span style='font-size:13px; color:black; font-weight:normal;'>R² = {r2:.4f}   |   P-Value = {pval:.4f}{sig_star}   |   Slope = {slope_stat:.4f} ± {slope_se:.4f} / yr</span>"
        
    change_point = compute_pettitt_test(valid_data)
    if change_point is not None and change_point in valid_data.index:
        ...
        slope1, _, _, _ = compute_trend_stats(seg1)
        if slope1 is not None:
            x_vals_1 = np.arange(len(seg1))
            trend_y1 = slope1 * x_vals_1 + (seg1.mean() - slope1 * x_vals_1.mean())
            fig.add_trace(go.Scatter(x=seg1.index, y=trend_y1, mode='lines', name='Pre-Shift Trend', line=dict(color='#ea580c', width=2, dash='dash')))
            
        slope2, _, _, _ = compute_trend_stats(seg2)
        if slope2 is not None:
            x_vals_2 = np.arange(len(seg2))
            trend_y2 = slope2 * x_vals_2 + (seg2.mean() - slope2 * x_vals_2.mean())
            fig.add_trace(go.Scatter(x=seg2.index, y=trend_y2, mode='lines', name='Post-Shift Trend', line=dict(color='#b91c1c', width=2, dash='dash')))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=dict(text=f"<b>{x_axis_title}</b>{stats_text}", font=dict(size=18, color='black', family='Arial')),
...
def compute_pettitt_test(series: pd.Series):
    """Calculates Pettitt's test for change point detection. Returns the index of the change if p < 0.05."""
    valid = series.dropna()
    n = len(valid)
    if n < 10: return None
    ranks = valid.rank()
    U = 2 * ranks.cumsum() - np.arange(1, n + 1) * (n + 1)
    K = U.abs().max()
    change_idx = U.abs().idxmax()
    ...
    try:
        ...
        if run_button:
            ...
            with main_tab5:
                ...
                st.caption("Estimates the probability of extreme precipitation events (like 1-in-50-year floods) using the Generalized Extreme Value distribution on Annual Maximum 1-Day Precipitation (RX1day).")
                ...
                if 'RX1day' in results.columns and not results['RX1day'].empty:
                    ...
                    if len(rx1day_data) > 10:
                        ...
                        gev_fig.update_layout(
                            title="Precipitation Return Periods (RX1day)",
                            xaxis_title="Return Period (Years)",
                            yaxis_title="Expected RX1day (mm)",
                            xaxis=dict(type='log', tickvals=[2, 5, 10, 20, 50, 100], ticktext=['2', '5', '10', '20', '50', '100']),
