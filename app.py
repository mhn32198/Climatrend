    ...
    try:
        ...
        if run_button:
            ...
            with main_tab5:
                ...
                st.info("Ask questions about your climate data in plain English. The AI can query, summarize, and describe patterns in your dataset.")

                if not PANDASAI_AVAILABLE:
                    st.error("""
                    **PandasAI is not installed.** Run this command in your terminal to enable the AI tab:
                    ```
                    ...
                    if openai_key:
                        try:
                            llm = PandasAI_OpenAI(api_token=openai_key)

                            if ai_target == "Calculated ETCCDI Indices" and 'results' in dir():
                                target_df = results.reset_index()
                                context_note = "ETCCDI climate indices table (annual or monthly values per index)"
                            else:
                                target_df = df[['YEAR', 'MONTH', 'DAY', 'TMAX', 'TMIN', 'PRCP']].copy()
                                context_note = "daily climate station data with TMAX, TMIN, and PRCP columns"

                            smart_df = SmartDataframe(
                                target_df,
                                config={"llm": llm, "verbose": False, "open_charts": False}
                            )
                            ...
                            user_query = st.text_input(
                                f"Ask a question about the {context_note}:",
                                placeholder="e.g. 'What year had the highest TMAX?' or 'Show the trend of R95p over time'"
                            )

                            if user_query:
                                with st.spinner("🌍 AI is analyzing your climate patterns..."):
                        ...
                        st.warning("Please enter your OpenAI API key above to activate the AI assistant.")
